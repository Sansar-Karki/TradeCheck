// -----------------------------
    ...
    function setTab(tab){
      ...
      $("topTitle").textContent = tab === "core" ? "Core" : tab[0].toUpperCase()+tab.slice(1);
      window.scrollTo({top:0, behavior:"smooth"});
      if(tab==="insights") { renderInsights(); requestAnimationFrame(()=>{ try{ if(chartR && chartR.resize) chartR.resize(); Object.values(window.__charts||{}).forEach(c=>c&&c.resize&&c.resize()); }catch(e){} }); }
      renderGoals();
      renderDailyHistory();
    ...
    const MISTAKE_TAGS = [
      ...
      "No plan", "Moved stop (mental)", "Traded chop", "Didn’t wait retest", "Overtraded"
    ];
    ...
    function saveReview(){
      ...
      renderReviewTab();
      renderGoals();
      if(!views.insights.classList.contains("hidden")) renderInsights();
      renderGoals();
      renderDailyHistory();
    }
...
function openDailyReview(){
    ...
    function saveDailyReview(){
      ...
      if(text.length < 10){ $("drHint").textContent = "Write at least one sentence."; return; }
      S.dailyReviews[todayISO()] = { text, ts: Date.now() };
      saveSmart(S);
      
      $("drHint").textContent = "Saved.";
    ...
    function renderReviewTab(){
      ...
      $("reviewHeadline").textContent = `${latest.symbol} • ${latest.strategyName}`;
      $("reviewSub").textContent = latest.reviewed ? `Reviewed • ${latest.review.r.toFixed(2)}R` : "Not reviewed yet";
      ...
      S.trades.slice(0,20).forEach(t=>{
        ...
        const rtxt = (t.reviewed && t.review) ? `${t.review.r.toFixed(2)}R` : "—";
        div.innerHTML = `
          <div class="flex items-center justify-between gap-3">
            <div>
              ...
              <div class="text-sm muted2">${escapeHtml(t.setup||"")} • ${t.date} • 📷 ${(t.attachments||[]).length}</div>
            </div>
    ...
    function renderTodayTrades(){
      ...
      trades.slice(0,10).forEach(t=>{
        ...
        const left = `
          <div>
            <div class="text-sm font-semibold">${escapeHtml(t.symbol)} <span class="text-sm muted2">• ${escapeHtml(t.strategyName)}</span></div>
            <div class="text-sm muted2">${escapeHtml(t.setup||"")} • ${escapeHtml(t.session||"")}</div>
          </div>
        `;
        const right = `
          <div class="text-right">
            <div class="text-sm font-semibold">${t.reviewed && t.review ? t.review.r.toFixed(2)+"R" : "Pending"}</div>
            <div class="text-xs muted2">${t.preScore!=null ? "Checklist "+t.preScore+"% ("+t.preGrade+")" : "Checklist not saved"} • 📷 ${(t.attachments||[]).length}</div>
          </div>
        `;
    ...
    // -----------------------------
    let chartR = null;
    let activeFilters = { strategyId:"", days:30, grade:"D" };

    function gradeMinPass(grade, min){
      const order = { "A":4, "B":3, "C":2, "D":1 };
      return (order[grade]||1) >= (order[min]||1);
    }

    function filteredReviewedTrades(){
      const days = parseInt(activeFilters.days||30,10);
      const cutoff = Date.now() - days*24*60*60*1000;
      return S.trades
        .filter(t=> t.reviewed && t.review && t.review.ts >= cutoff)
        .filter(t=> !activeFilters.strategyId || t.strategyId === activeFilters.strategyId)
        .filter(t=> gradeMinPass(t.preGrade || "D", activeFilters.grade || "D"))
        .slice()
        .reverse();
    }
    ...
    function renderInsights(){
      // filters UI
      syncStrategySelects();
      $("fltDays").value = String(activeFilters.days || 30);
      $("fltGrade").value = activeFilters.grade || "D";
      $("fltStrategy").value = activeFilters.strategyId || "";

      const reviewed = filteredReviewedTrades();

      // R curve data
      let cum = 0;
      const labels = [];
      const data = [];
      reviewed.forEach((t,i)=>{
        cum += (t.review?.r || 0);
        labels.push(t.date);
        data.push(cum);
      });
      ...
      if(chartR){
        chartR.destroy();
        chartR = null;
        try{ window.__charts.chartR = null; }catch(e){}
      }
    ...
    $("btnApplyFilters").addEventListener("click", ()=>{
      activeFilters.strategyId = $("fltStrategy").value || "";
      activeFilters.days = clamp(parseInt($("fltDays").value||"30",10), 1, 365);
    ...
    $("btnResetFilters").addEventListener("click", ()=>{
      activeFilters = { strategyId:"", days:30, grade:"D" };
      renderInsights();
      renderGoals();
      renderDailyHistory();
      toast("Filters reset");
...
$("btnSaveReview").addEventListener("click", saveReview);
    ...
    function maybeNudgeDailyReview(){
      ...
      if(S.notifs.browser && "Notification" in window && Notification.permission==="granted"){
        try{ new Notification("TradeCheck", { body: "Don’t forget your Daily Review." }); }catch(e){}
      }
    }
